...
const PREFERRED_GEMINI_MODELS = ['gemini-3.5-flash-lite', 'gemini-3.6-flash', 'gemini-3.5-flash'];

type ChatRequest = { message?: unknown; chatId?: unknown; documentId?: unknown };
...
export default async function handler(request: VercelRequest, response: ServerResponse): Promise<void> {
  ...
  try {
    ...
    const client = createClient<Database>(supabaseUrl, supabaseAnonKey, { auth: { persistSession: false, autoRefreshToken: false }, global: { headers: { Authorization: `Bearer ${token}` } } });
    ...
    const documentText = documentId ? await loadPdfText(client, userId, documentId, geminiApiKey, model) : null;
